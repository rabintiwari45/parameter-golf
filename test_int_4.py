...
_MAG_QUANT_LZMA = b"\xfdLZ"
_MAG_QUANT_ZLIB = b"\xfdZL"
...
def compress_quant_pickle_bytes(raw: bytes) -> bytes:
    ...
    if codec in ("raw", "none", "identity"):
        return raw
    if codec == "lzma":
        preset = max(0, min(9, int(os.environ.get("LZMA_PRESET", "6"))))
        return _MAG_QUANT_LZMA + lzma.compress(raw, format=lzma.FORMAT_XZ, preset=preset)
    level = max(1, min(9, int(os.environ.get("ZLIB_LEVEL", "9"))))
    return _MAG_QUANT_ZLIB + zlib.compress(raw, level=level)
...
def pack_bool_mask_bitwise(mask: Tensor) -> Tensor:
    ...
    if pad_n:
        m = torch.cat(
            [m, torch.zeros(pad_n, dtype=torch.bool, device=m.device)],
            dim=0,
        )
    bits = m.view(-1, 8).to(torch.uint8)
    w = (1 << torch.arange(8, device=bits.device, dtype=torch.uint8)).view(1, 8)
    return (bits * w).sum(dim=1).to(torch.uint8)


def unpack_bool_mask_bitwise(packed: Tensor, rows: int, cols: int) -> Tensor:
    n = rows * cols
    p = packed.to(torch.long)
    flat = torch.stack([(p >> i) & 1 for i in range(8)], dim=1).flatten()
...
def dequantize_state_dict_nf4(obj):
    ...
    def _scale_to_grouped(scale: Tensor, rows: int, num_groups: int) -> Tensor:
        ...
        if sc.ndim == 0:
            ...
            sc = sc[:, None] if num_groups == 1 else sc.reshape(rows, num_groups)
...
def main() -> None:
    ...
    quant_blob = compress_quant_pickle_bytes(quant_raw)
    ...
    if master_process:
        ...
        code_bytes = len(code.encode("utf-8"))
        # ratio = quant_stats["baseline_tensor_bytes"] / max(quant_stats["int4_payload_bytes"], 1)
        # log0(
        #     f"Serialized model int8+zlib: {quant_file_bytes} bytes "
        #     f"(payload:{quant_stats['int4_payload_bytes']} raw_torch:{quant_raw_bytes} payload_ratio:{ratio:.2f}x)"
        # )
